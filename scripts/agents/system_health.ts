...
const parts=[
  "loops","second_order","socio_tech_alignment","constraints_report","resilience_index","multi_agent_sync"
];
for(const p of parts){
  fs.writeFileSync(`reports/system/${p}.md`, `# ${p}\nGenerated ${stamp()}\n`);
  fs.writeFileSync(`solutions/system/${p}_solutions.md`, `# Solutions for ${p}\n`);
}
fs.writeFileSync(`reports/system_health_${stamp()}.md`,
`# System Health Master Report (${stamp()})
- Modules: ${parts.join(", ")}
